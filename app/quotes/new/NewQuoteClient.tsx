...
export default function NewQuoteClient({ loadId }: { loadId?: string }) {
  ...
  useEffect(() => {
    ...
    // Refresh from DB
    loadClientsFromDb()
      .then((fresh) => setClients(fresh.map((c) => ({ id: c.id, name: c.name }))))
      .catch(console.warn);
    if (loadId) {
      loadQuotesFromDb()
        .then((qs) => {
          const existing = qs.find((q) => q.id === loadId);
          if (existing) setQuote(existing);
        })
        .catch(console.warn);
    }
  ...
  async function save() {
    ...
    try {
      ...
      console.warn(e);
      alert(lang === 'ca'
        ? 'No s\'ha pogut desar a la BBDD.'
        : 'No se ha podido guardar en la base de datos.');
    }
  }
  ...
  async function onImportXlsx(file: File) {
    ...
    const newLines: QuoteLine[] = (rows || [])
      .map((r, idx) => {
        const concept = String(r.concept ?? r.Concepto ?? r.Concepte ?? r.descripcion ?? r.descripcio ?? `Línia ${idx + 1}`).trim();
        const qty = parseNumber(r.qty ?? r.Qty ?? r.quantitat ?? r.cantidad ?? r.unitats ?? r.uts ?? 1);
        const unitCostGross = parseNumber(r.unitCost ?? r.cost ?? r.precio ?? r.preu ?? r['€/Ut.'] ?? r['€/ud'] ?? 0);
        const discountPercent = parseNumber(r.discount ?? r.desc ?? r['Desc.'] ?? 0);
        const marginPercent = parseNumber(r.margin ?? 0);
        const category = String(r.category ?? r.categoria ?? 'Material');
        const typeRaw = String(r.type ?? 'product').toLowerCase();
        const type: QuoteLineType = (['product', 'service', 'transport', 'info'].includes(typeRaw) ? typeRaw : 'product') as QuoteLineType;
        return {
          id: uid('line'),
          type,
          concept,
    ...
    if (!newLines.length) {
      alert(lang === 'ca'
        ? 'No s\'han trobat línies a l\'Excel.'
        : 'No se han encontrado líneas en el Excel.');
      return;
    }
    setQuote((q) => ({ ...q, lines: newLines }));
  }
  ...
  async function onImportPdf(file: File) {
    ...
    if (!res.ok) {
      ...
      throw new Error(txt || 'PDF import failed');
    }
    const data = (await res.json()) as { lines?: any[]; vatPercent?: number };
    const lines = Array.isArray(data.lines) ? data.lines : [];
    const newLines: QuoteLine[] = lines.map((l: any) => ({
      id: uid('line'),
      type: (l.type ?? 'product') as QuoteLineType,
      category: String(l.category ?? 'Material'),
      concept: String(l.concept ?? ''),
      qty: parseNumber(l.qty ?? 1),
      unitCostGross: parseNumber(l.unitCostGross ?? 0),
      discountPercent: parseNumber(l.discountPercent ?? 0),
      marginPercent: parseNumber(l.marginPercent ?? 0)
    })).filter((l) => l.concept.trim().length > 0);

    if (!newLines.length) {
      alert(lang === 'ca'
        ? 'La IA no ha retornat línies del PDF. Prova amb un altre fitxer o revisa la configuració.'
        : 'La IA no ha devuelto líneas del PDF.');
      return;
    }

    setQuote((q) => ({
      ...q,
      vatPercent: typeof data.vatPercent === 'number' ? data.vatPercent : q.vatPercent,
      lines: newLines
    }));
  }
  ...
  async function onImportFile(file: File) {
    ...
    if (name.endsWith('.csv')) return onImportCsv(file);
    if (name.endsWith('.xlsx') || name.endsWith('.xls')) return onImportXlsx(file);
    if (name.endsWith('.pdf')) return onImportPdf(file);
    alert(lang === 'ca'
      ? 'Format no suportat. Usa CSV, Excel o PDF.'
      : 'Formato no soportado. Usa CSV, Excel o PDF.');
  }
      ...
      <div className="card">
        ...
        <div className="toast">
          ...
          <input
            className="input"
            type="file"
            accept=".csv,text/csv,.xlsx,.xls,application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,.pdf,application/pdf"
            onChange={(e) => {
              const f = e.target.files?.[0];
              if (f) {
                void onImportFile(f).catch((err) => {
                  console.warn(err);
                  alert(lang === 'ca'
                    ? 'Error important el fitxer. Si és un PDF, revisa que la IA estigui configurada.'
                    : 'Error importando el archivo. Si es PDF, revisa la configuración de IA.');
                });
              }
            }}
          />
